...
std::string get_name(uint8_t opcode)
{
    ...
    return (name != nullptr) ? name : "0x" + zvmc::hex(opcode);
}
...
class InstructionTracer : public Tracer
{
        ...
        zvmc_revision rev, const zvmc_message& msg, bytes_view code) noexcept override
    {
        ...
        m_out << R"(,"rev":")" << rev << '"';
        m_out << R"(,"static":)" << (((msg.flags & ZVMC_STATIC) != 0) ? "true" : "false");
        m_out << "}\n";
    }
        ...
        int64_t gas, const ExecutionState& state) noexcept override
    {
        ...
        // Full memory can be dumped as zvmc::hex({state.memory.data(), state.memory.size()}),
    ...
    void on_execution_end(const zvmc_result& result) noexcept override
    {
        ...
        m_out << R"(,"output":")" << zvmc::hex({result.output_data, result.output_size}) << '"';
        m_out << "}\n";
