...
public:
    ...
    void notify_execution_start(  // NOLINT(misc-no-recursion)
        zvmc_revision rev, const zvmc_message& msg, bytes_view code) noexcept
    {
    ...
    void notify_execution_end(const zvmc_result& result) noexcept  // NOLINT(misc-no-recursion)
    {
...
private:
    virtual void on_execution_start(
        zvmc_revision rev, const zvmc_message& msg, bytes_view code) noexcept = 0;
    ...
    virtual void on_execution_end(const zvmc_result& result) noexcept = 0;
};
...
ZVMC_EXPORT std::unique_ptr<Tracer> create_histogram_tracer(std::ostream& out);

ZVMC_EXPORT std::unique_ptr<Tracer> create_instruction_tracer(std::ostream& out);
