...
/// The size of the ZVM 256-bit word.
...
/// Grows ZVM memory and checks its cost.
///
...
///
/// These are minimal ZVM instruction implementations which assume:
/// - the stack requirements (overflow, underflow) have already been checked,
/// - the "base" gas const has already been charged,
/// - the `stack` pointer points to the ZVM stack top element.
...
template <zvmc_status_code Status>
inline TermResult stop_impl(
    StackTop /*stack*/, int64_t gas_left, ExecutionState& /*state*/) noexcept
{
    return {Status, gas_left};
}
inline constexpr auto stop = stop_impl<ZVMC_SUCCESS>;
inline constexpr auto invalid = stop_impl<ZVMC_INVALID_INSTRUCTION>;
...
{
    auto& x = stack.top();
    const auto addr = intx::be::trunc<zvmc::address>(x);

    if (state.host.access_account(addr) == ZVMC_ACCESS_COLD)
    {
...
{
    const auto addr = intx::be::trunc<zvmc::address>(stack.pop());
    const auto& mem_index = stack.pop();
    const auto& input_index = stack.pop();
    const auto& size = stack.pop();
...
inline Result returndatacopy(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    if (state.return_data.size() < input_index)
        return {ZVMC_INVALID_MEMORY_ACCESS, gas_left};
...
inline void blockhash(StackTop stack, ExecutionState& state) noexcept
{
    ...
    const auto header =
        (number < upper_bound && n >= lower_bound) ? state.host.get_block_hash(n) : zvmc::bytes32{};
...
{
    // TODO: introduce selfbalance in ZVMC?
...
inline code_iterator jump_impl(ExecutionState& state, const uint256& dst) noexcept
{
    ...
    {
        state.status = ZVMC_BAD_JUMP_DESTINATION;
        return nullptr;
    }
...
inline Result log(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    if (state.in_static_mode())
        return {ZVMC_STATIC_MODE_VIOLATION, 0};
    ...
    std::array<zvmc::bytes32, NumTopics> topics;  // NOLINT(cppcoreguidelines-pro-type-member-init)
    for (auto& topic : topics)
        topic = intx::be::store<zvmc::bytes32>(stack.pop());
...
template <zvmc_status_code StatusCode>
...
inline constexpr auto return_ = return_impl<ZVMC_SUCCESS>;
inline constexpr auto revert = return_impl<ZVMC_REVERT>;
