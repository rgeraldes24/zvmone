...
/// The view/controller for ZVM stack.
class Stack
{
public:
...
struct AdvancedExecutionState : ExecutionState
{
    ...
    AdvancedExecutionState(const zvmc_message& message, zvmc_revision revision,
    ...
    const Instruction* exit(zvmc_status_code status_code) noexcept
    {
        status = status_code;
        return nullptr;
    }
...
ZVMC_EXPORT AdvancedCodeAnalysis analyze(zvmc_revision rev, bytes_view code) noexcept;
