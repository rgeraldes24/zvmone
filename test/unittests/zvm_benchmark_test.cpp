...
/// This file contains ZVM code generators for micro benchmarks,
...
TEST_P(zvm, grow_memory_with_mload)
{
...
TEST_P(zvm, grow_memory_with_mstore)
{
    ...
    // Pokes the same offset 0 all the time.
    execute(code, 0x00_bytes32);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 61452);

    // Pokes memory offset increasing by 1, memory grows every 32nd "iteration".
    execute(code, 0x01_bytes32);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 61868);

    // Pokes memory offset increasing by 32, memory grows every "iteration".
    execute(code, 0x20_bytes32);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 106505);
}

TEST_P(zvm, jump_around)
{
    ...
    execute(code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, int64_t{(1 + 3 + 8) * num_jumps + 1});
}

TEST_P(zvm, signextend_bench)
{
    ...
    execute(code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 8 * num_instr + 15 * 3 + 21);
    EXPECT_OUTPUT_INT(0);
}
