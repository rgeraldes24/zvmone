...
/// This file contains ZVM unit tests that access or modify the contract storage.
...
TEST_P(zvm, sload_cost)
{
    rev = ZVMC_SHANGHAI;
    execute(2106, sload(dup1(0)));
    EXPECT_GAS_USED(ZVMC_SUCCESS, 2106);
    EXPECT_EQ(host.accounts[msg.recipient].storage.size(), 1);
}

TEST_P(zvm, sstore_out_of_block_gas)
{
    ...
    execute(22111, code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 22111);
...
TEST_P(zvm, sstore_cost_net_gas_metering)
{
    ...
    const auto test = [this](const zvmc::bytes32& original, const zvmc::bytes32& current,
                          const zvmc::bytes32& value, int64_t expected_gas_used,
        ...
        storage_entry.access_status = ZVMC_ACCESS_WARM;
        execute(sstore(key, value));
        EXPECT_EQ(storage_entry.current, value);
        EXPECT_GAS_USED(ZVMC_SUCCESS, expected_gas_used);
        EXPECT_EQ(result.gas_refund, expected_gas_refund);
    };

    std::array<CostConstants, ZVMC_MAX_REVISION + 1> cost_constants{};
    cost_constants[ZVMC_SHANGHAI] = {100, 20000, 2900, 4800};
...
TEST_P(zvm, sstore_below_stipend)
{
