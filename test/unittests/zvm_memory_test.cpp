...
TEST_P(zvm, memory_and_not)
{
    ...
    EXPECT_GAS_USED(ZVMC_SUCCESS, 42);
...
TEST_P(zvm, memory_big_allocation)
{
...
TEST_P(zvm, memory_grow_mstore8)
{
    ...
    execute(code, zvmc::bytes32{size});
    EXPECT_STATUS(ZVMC_SUCCESS);
...
TEST_P(zvm, mstore8_memory_cost)
{
...
TEST_P(zvm, keccak256_memory_cost)
{
...
TEST_P(zvm, calldatacopy_memory_cost)
{
    const auto code = calldatacopy(0, 0, 1);
    execute(18, code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 18);
}
...
TEST_P(zvm, memory_access)
{
    ...
    for (const auto& p : memory_access_test_cases)
    {
        for (auto& t : memory_access_opcodes)
        {
            ...
            {
                EXPECT_EQ(result.status_code, (t.opcode == OP_REVERT) ? ZVMC_REVERT : ZVMC_SUCCESS)
                    << case_descr;
            ...
            else
            {
                if (t.opcode == OP_RETURNDATACOPY)
                {
                    ...
                    EXPECT_TRUE(result.status_code == ZVMC_OUT_OF_GAS ||
                                result.status_code == ZVMC_INVALID_MEMORY_ACCESS);
                }
                else
                {
                    EXPECT_EQ(result.status_code, ZVMC_OUT_OF_GAS) << case_descr;
                }
