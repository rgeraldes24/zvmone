...
{
    execute(0, {});
    EXPECT_GAS_USED(ZVMC_SUCCESS, 0);

    execute(1, {});
    EXPECT_GAS_USED(ZVMC_SUCCESS, 0);
}

TEST_P(zvm, push_and_pop)
{
    execute(11, push("0102") + OP_POP + push("010203040506070809") + OP_POP);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 10);
}

TEST_P(zvm, push_implicit_data)
{
    ...
    for (auto op = uint8_t{OP_PUSH1}; op <= OP_PUSH32; ++op)
    {
        code.back() = op;
        execute(code);
        EXPECT_GAS_USED(ZVMC_SUCCESS, 307);
    }
}

TEST_P(zvm, stack_underflow)
{
    execute(13, push(1) + OP_POP + push(1) + OP_POP + OP_POP);
    EXPECT_STATUS(ZVMC_STACK_UNDERFLOW);
...
TEST_P(zvm, add)
{
    ...
    EXPECT_GAS_USED(ZVMC_SUCCESS, 24);
...
TEST_P(zvm, dup)
{
    ...
    EXPECT_GAS_USED(ZVMC_SUCCESS, 48);
    EXPECT_OUTPUT_INT(20);
}

TEST_P(zvm, dup_all_1)
{
...
TEST_P(zvm, dup_stack_underflow)
{
    for (int i = 0; i < 16; ++i)
    {
...
TEST_P(zvm, swapsn_jumpdest)
{
    ...
    execute(code);
    EXPECT_STATUS(ZVMC_SUCCESS);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 30);

    rev = ZVMC_MAX_REVISION;
    execute(code);
    EXPECT_STATUS(ZVMC_SUCCESS);
}

TEST_P(zvm, swapsn_push)
{
    // Test demonstrating possible problem with introducing multibyte SWAP/DUP instructions as per
    // EIP-663 variants B and C.
    ...
    rev = ZVMC_SHANGHAI;
    execute(code);
    EXPECT_STATUS(ZVMC_BAD_JUMP_DESTINATION);
...
TEST_P(zvm, addmod_mulmod_by_zero)
{
...
TEST_P(zvm, signextend)
{
    ...
    s += "60406000f3";  // RETURN(0,64)
    execute(49, s);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 49);
    ASSERT_EQ(result.output_size, 64);
...
TEST_P(zvm, exp)
{
    ...
    execute(131, code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 131);
    ASSERT_EQ(result.output_size, 32);
...
TEST_P(zvm, exp_1_0)
{
    ...
    execute(31, code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 31);
    EXPECT_OUTPUT_INT(1);
}
...
{
    execute(1, "2a");
    EXPECT_EQ(result.status_code, ZVMC_UNDEFINED_INSTRUCTION);
    EXPECT_EQ(result.gas_left, 0);
}
...
{
    execute(1, "fe");
    EXPECT_EQ(result.status_code, ZVMC_INVALID_INSTRUCTION);
    EXPECT_EQ(result.gas_left, 0);
}

TEST_P(zvm, inner_stop)
{
    const auto code = push(0) + OP_STOP + OP_POP;
    execute(3, code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 3);
}

TEST_P(zvm, inner_return)
{
    const auto code = ret(0, 0) + push(0);
    execute(6, code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 6);
}

TEST_P(zvm, inner_revert)
{
    const auto code = revert(0, 0) + push(0);
    execute(6, code);
    EXPECT_GAS_USED(ZVMC_REVERT, 6);
}

TEST_P(zvm, inner_invalid)
{
    const auto code = push(0) + "fe" + OP_POP;
    execute(5, code);
    EXPECT_GAS_USED(ZVMC_INVALID_INSTRUCTION, 5);
}

TEST_P(zvm, keccak256)
{
    execute(push(0x0800) + push(0x03ff) + OP_KECCAK256 + ret_top());
    EXPECT_GAS_USED(ZVMC_SUCCESS, 738);
    EXPECT_OUTPUT_INT(0xaeffb38c06e111d84216396baefeb7fed397f303d5cb84a33f1e8b485c4a22da_u256);
}

TEST_P(zvm, keccak256_empty)
{
...
TEST_P(zvm, revert)
{
    ...
    EXPECT_EQ(result.status_code, ZVMC_REVERT);
    ASSERT_EQ(result.output_size, 2);
    EXPECT_EQ(result.output_data[0], 0);
    EXPECT_EQ(result.output_data[1], 0xee);
}

TEST_P(zvm, return_empty_buffer_at_offset_0)
{
    execute(dup1(OP_MSIZE) + OP_RETURN);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 5);
}

TEST_P(zvm, return_empty_buffer_at_high_offset)
{
...
TEST_P(zvm, undefined_instructions)
{
    for (auto i = 0; i <= ZVMC_MAX_REVISION; ++i)
    {
        const auto r = zvmc_revision(i);
        for (uint8_t opcode = 0; opcode <= 0xfe; ++opcode)
        {
            if (zvmone::instr::gas_costs[r][opcode] != zvmone::instr::undefined)
                continue;

            auto res = vm.execute(host, r, {}, &opcode, sizeof(opcode));
            EXPECT_EQ(res.status_code, ZVMC_UNDEFINED_INSTRUCTION)
                << " for opcode " << hex(opcode) << " on revision " << r;
        }
    }
}

TEST_P(zvm, undefined_instruction_analysis_overflow)
{
    rev = ZVMC_SHANGHAI;
...
TEST_P(zvm, undefined_instruction_block_cost_negative)
{
    // For undefined instructions ZVMC instruction tables have cost -1.
    ...
    EXPECT_STATUS(ZVMC_UNDEFINED_INSTRUCTION);
...
TEST_P(zvm, abort)
{
    for (auto r = 11; r <= ZVMC_MAX_REVISION; ++r)
    {
        auto opcode = uint8_t{0xfe};
        auto res = vm.execute(host, zvmc_revision(r), {}, &opcode, sizeof(opcode));
        EXPECT_EQ(res.status_code, ZVMC_INVALID_INSTRUCTION);
    }
}

TEST_P(zvm, staticmode)
{
        ...
        {OP_SSTORE, OP_LOG0, OP_LOG1, OP_LOG2, OP_LOG3, OP_LOG4, OP_CALL, OP_CREATE, OP_CREATE2})
    {
        ...
        EXPECT_EQ(result.status_code, ZVMC_STATIC_MODE_VIOLATION) << hex(op);
        EXPECT_EQ(result.gas_left, 0);
    }
}

TEST_P(zvm, max_code_size_push1)
{
...
TEST_P(zvm, reverse_16_stack_items)
{
