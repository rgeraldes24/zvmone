...
TEST_P(zvm, jumpi_else)
{
    execute(16, dup1(OP_COINBASE) + OP_JUMPI);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 15);
    EXPECT_EQ(result.output_size, 0);
}

TEST_P(zvm, jumpi_at_the_end)
{
...
TEST_P(zvm, bad_jumpdest)
{
    ...
    for (auto op : {OP_JUMP, OP_JUMPI})
    {
        ...
        EXPECT_EQ(result.status_code, ZVMC_BAD_JUMP_DESTINATION);
        EXPECT_EQ(result.gas_left, 0);
...
TEST_P(zvm, jump_to_block_beginning)
{
...
TEST_P(zvm, jump_over_jumpdest)
{
    ...
    execute(code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 3 + 8 + 1);
}

TEST_P(zvm, jump_to_missing_push_data)
{
    execute(push(5) + OP_JUMP + OP_PUSH1);
    EXPECT_STATUS(ZVMC_BAD_JUMP_DESTINATION);
}

TEST_P(zvm, jump_to_missing_push_data2)
{
    execute(push(6) + OP_JUMP + OP_PUSH2 + "ef");
    EXPECT_STATUS(ZVMC_BAD_JUMP_DESTINATION);
}

TEST_P(zvm, jump_dead_code)
{
    execute(push(6) + OP_JUMP + 3 * OP_INVALID + OP_JUMPDEST);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 12);
}

TEST_P(zvm, stop_dead_code)
{
    execute(OP_STOP + 3 * OP_INVALID + OP_JUMPDEST);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 0);
}

TEST_P(zvm, dead_code_at_the_end)
{
    execute(OP_STOP + 3 * OP_INVALID);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 0);
}

TEST_P(zvm, jumpi_jumpdest)
{
    const auto code = calldataload(0) + push(6) + OP_JUMPI + OP_JUMPDEST;

    execute(code, "00"_hex);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 20);

    execute(code, "ff"_hex);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 20);
}

TEST_P(zvm, jumpi_followed_by_stack_underflow)
{
