...
/// This file contains non-mainstream ZVM unit tests not matching any concrete category:
/// - regression tests,
/// - tests from fuzzers,
/// - zvmone's internal tests.

#include "zvm_fixture.hpp"

using zvmone::test::zvm;

TEST_P(zvm, zvmone_loaded_program_relocation)
{
    // The bytecode of size 2 will create zvmone's loaded program of size 4 and will cause
...
TEST_P(zvm, zvmone_block_stack_req_overflow)
{
...
TEST_P(zvm, zvmone_block_max_stack_growth_overflow)
{
...
TEST_P(zvm, zvmone_block_gas_cost_overflow_create)
{
...
TEST_P(zvm, zvmone_block_gas_cost_overflow_balance)
{
    // Here we build single-block bytecode with as many BALANCE instructions as possible.

    rev = ZVMC_SHANGHAI;  // Here BALANCE costs 100 or 2600; depends on whether the account is warm
                          // or not.
...
TEST_P(zvm, loop_full_of_jumpdests)
{
    ...
    execute(code);
    EXPECT_GAS_USED(ZVMC_SUCCESS, 7987882);
}

TEST_P(zvm, jumpdest_with_high_offset)
{
    for (auto offset : {3u, 16383u, 16384u, 32767u, 32768u, 65535u, 65536u})
    {
        ...
        execute(code);
        EXPECT_EQ(result.status_code, ZVMC_SUCCESS) << "JUMPDEST at " << offset;
    }
}
